...
- 📋 **详情视图（核心）**：
  ...
  - 团队评论区（任意成员用当前操作人身份发帖）
  - 统一活动记录（状态变更 + 评论合并时间线）
- 👥 **团队协作**：左侧边栏「当前操作人」选择器（李娜-采购经理、张伟-法务等预设 + 自定义）。评论、附件上传、默认负责人均自动带上该人名。适合内部团队共享使用（无真实权限墙）
...
**日常使用流程**（强烈推荐）：
1. 左侧边栏顶部选择「当前操作人」（李娜-采购经理 等）—— 这会影响所有评论、上传、默认负责人。
2. 查看「仪表盘」了解整体进度。
...
- 通过左侧「当前操作人」切换身份，即可模拟不同同事发评论/上传文件。
- 所有数据实时共享（SQLite + 文件）。
...
3. **详情视图（最重要）**：
   ...
   - 团队评论（当前操作人自动署名）
   - 统一活动时间线（状态变更 + 评论）
4. **添加供应商**：标准表单（国家、负责人默认当前操作人）。

**团队协作核心**：边栏「当前操作人」选择器 → 贯穿评论、附件、默认 owner、历史备注。非常适合小团队在共享文件夹上一起使用。

## 自定义 / Customization

- 团队成员列表：`utils/constants.py` 的 `ACTORS`
- 状态 / 平台 / 国家：同文件对应常量
- 翻译：`utils/i18n.py`
